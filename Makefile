...
STATIC_DRIVER_COPY_TARGET := scylladb_static.lib_copy
else
STATIC_DRIVER_COPY_TARGET := libscylladb_static.a_copy
endif
STATIC_DRIVER_BUILD_TARGETS := scylladb_staticlib_target $(STATIC_DRIVER_COPY_TARGET)
...
test-package-deb: build-package
	...
	$(SMOKE_TEST_MAKE) verify-driver-dev-deb
	$(SMOKE_TEST_MAKE) remove-driver-dev-deb || true
	$(SMOKE_TEST_MAKE) remove-driver-deb || true
	$(SMOKE_TEST_MAKE) install-driver-dev-deb
	$(call smoke_test_build_package,DEB)
	$(SMOKE_TEST_MAKE) install-app-deb SCYLLA_SMOKE_BUILD_STATIC=$(SCYLLA_SMOKE_BUILD_STATIC)
...
test-package-rpm: build-package
	...
	docker run --rm \
		...
		bash -c ' \
			...
			$(SMOKE_TEST_MAKE) verify-driver-dev-rpm; \
			$(SMOKE_TEST_MAKE) remove-driver-dev-rpm || true; \
			$(SMOKE_TEST_MAKE) remove-driver-rpm || true; \
...
test-package-rpm-native: build-package
	...
	$(call smoke_test_build_package,RPM)
	$(SMOKE_TEST_MAKE) install-app-rpm
	$(call smoke_test_run_package,SCYLLA_HOST=$(SCYLLA_HOST) SKIP_DOCKER_COMPOSE=$(SKIP_DOCKER_COMPOSE)) SCYLLA_SMOKE_BUILD_STATIC=$(SCYLLA_SMOKE_BUILD_STATIC)
	@echo "=== RPM package test completed successfully ==="
	$(SMOKE_TEST_MAKE) remove-app-rpm || true
	$(SMOKE_TEST_MAKE) remove-driver-dev-rpm || true
	$(SMOKE_TEST_MAKE) remove-driver-rpm || true
